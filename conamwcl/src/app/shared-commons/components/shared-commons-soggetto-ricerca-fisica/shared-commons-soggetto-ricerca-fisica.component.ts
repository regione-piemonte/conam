...
@Component({
  selector: "shared-commons-soggetto-ricerca-fisica",
  templateUrl: "./shared-commons-soggetto-ricerca-fisica.component.html",
})
@DestroySubscribers()
export class SharedCommonsSoggettoRicercaFisicaComponent
  ...
  ngOnInit(): void {
    this.logger.init(SharedCommonsSoggettoRicercaFisicaComponent.name);
    if (!this.modalita) {
      this.modalita = "R";
  ...
  ngOnChanges() {
    this.logger.change(SharedCommonsSoggettoRicercaFisicaComponent.name);
    this.copySoggetto = JSON.parse(JSON.stringify(this.soggetto));
    this.manageLuoghiBySoggetto();
  }

  ngOnDestroy(): void {
    this.logger.destroy(SharedCommonsSoggettoRicercaFisicaComponent.name);
  }
}
