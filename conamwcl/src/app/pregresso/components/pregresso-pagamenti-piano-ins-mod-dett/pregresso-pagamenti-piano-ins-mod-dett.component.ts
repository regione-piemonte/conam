...
@Component({
  selector: "pregresso-pagamenti-piano-ins-mod-dett",
  templateUrl: "./pregresso-pagamenti-piano-ins-mod-dett.component.html",
  styleUrls: ["./pregresso-pagamenti-piano-ins-mod-dett.component.scss"],
})
export class PregressoPagamentiPianoInsModDettComponent
  ...
  ngOnInit(): void {
    this.logger.init(PregressoPagamentiPianoInsModDettComponent.name);
  ...
  loadPiano() {
    if (!this.isModifica) {
      ...
      this.enableSave =
        this.piano.stato && this.piano.stato.id == 1 ? true : false;
  ...
  onChangeDataEmitter(reload: boolean = false) {
    let obj: any = {};
    obj.disabled = !this.isRataCalcolata;
  ...
  salva() {
    this.loaded = false;
    let saveRequest = new SalvaPianoPregressiRequest(
      this.idVerbale,
      this.idOrdinanza
    );
    saveRequest.piano = this.piano;
    this.subscribers.salva = this.pregressoVerbaleService
      .salvaPiano(saveRequest)
      .subscribe(
        (data) => {
          this.loaded = true;
          // ricaricare stati
          this.onChangeDataEmitter(true);
        },
        (err) => {
          if (err instanceof ExceptionVO) {
            this.manageMessageTop(err.message, err.type);
