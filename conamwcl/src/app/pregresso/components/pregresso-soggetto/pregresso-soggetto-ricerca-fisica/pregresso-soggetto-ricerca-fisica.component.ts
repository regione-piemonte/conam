...
@Component({
  selector: "pregresso-soggetto-ricerca-fisica",
  templateUrl: "./pregresso-soggetto-ricerca-fisica.component.html",
})
@DestroySubscribers()
export class PregressoSoggettoRicercaFisicaComponent
  ...
  ngOnInit(): void {
    this.logger.init(PregressoSoggettoRicercaFisicaComponent.name);
    if (!this.modalita) {
      this.modalita = "R";
  ...
  ngOnChanges() {
    this.logger.change(PregressoSoggettoRicercaFisicaComponent.name);
    this.copySoggetto = JSON.parse(JSON.stringify(this.soggetto));
    this.manageLuoghiBySoggetto();
  }

  ngOnDestroy(): void {
    this.logger.destroy(PregressoSoggettoRicercaFisicaComponent.name);
  }
}
