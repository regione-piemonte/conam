...
export class PagamentiRiconciliaVerbaleAllegatoComponent implements OnInit, OnDestroy {
    ...
    salvaAllegato(nuovoAllegato: SalvaAllegatoVerbaleRequest) {
        ...
        this.loaded = false;
        this.subscribers.salvaAllegato = this.sharedVerbaleService.creaAllegatoVerbale(nuovoAllegato).subscribe(data => {
            let azione: string = "allegato";
            this.router.navigate([Routing.PAGAMENTI_RICONCILIA_VERBALE_RIEPILOGO + this.idVerbale, { action: azione }]);
            this.loaded = true;
        }, err => {
            if (err instanceof ExceptionVO) {
                this.manageMessage(err.type, err.message);
