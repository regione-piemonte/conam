...
export class PagamentiPianoViewComponent implements OnInit, OnDestroy {
  ...
  inviaRichiestaBollettini() {
    this.loaded = false;
    this.pagamentiService.inviaRichiestaBollettini(this.idPiano).subscribe(
      (data) => {
        this.loadPiano();
        this.loaded = true;
  ...
  scaricaBollettini() {
    ...
    this.pagamentiService.downloadBollettini(this.idPiano).subscribe(
      (data) => {
        let num: string =
          this.piano.numProtocollo != null ? this.piano.numProtocollo : "";
  ...
  goToInserisciNotifica() {
    this.router.navigateByUrl(
      Routing.PAGAMENTI_PIANO_INS_NOTIFICA + this.idPiano
    );
  }

  goToVisualizzaNotifica() {
    this.router.navigateByUrl(
      Routing.PAGAMENTI_PIANO_VIEW_NOTIFICA + this.idPiano
    );
  }
