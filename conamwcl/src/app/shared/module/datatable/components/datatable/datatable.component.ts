...
export class DataTableComponent implements OnInit, OnChanges, OnDestroy {
  ///////////////////
  @Input() public showMultiselect?: boolean = false;

  //////
  // bottone da abilitare per il download complessivo dei dati
  @Input() public showDownloadReport?: string = "";
  ...
  //manage the case of the default selection of the first element in checkbox table
  @Input() public defaultFirstElSelected?: boolean = false;
  //data to show as selected
  @Input() public defaultDataSelected?: any[] = [];
  ...
  @Output("onDownloadList") public onDownloadList? = new EventEmitter<any>();
  ...
  //nel caso in cui vengano mostrate le colonne dinamiche, controllo nel caso
  //non vengano selezionate colonne
  private controlNotEmptyChecked: boolean = true;
  ...
  public pagination: any = {
    ...
    change: (i) => {
      ...
      this.pagination.endIndex =
        this.pagination.page * this.pagination.perPage - 1;
    ...
    showIndex: (i) =>
      i >= this.pagination.startIndex && i <= this.pagination.endIndex,
    range: (startAt: number = 0, endAt: number = 0) =>
      Array(endAt - startAt + 1)
        .fill(0)
        .map((i, index) => startAt + index),
    getPages: (totalNumber?) =>
      this.pagination
        .range(-this.pagination.mRange, this.pagination.mRange)
    ...
    allowNext: (totalNumber: number): boolean => {
      if (!totalNumber) totalNumber = this.pagination.getSourceLength();
      return (
        this.pagination.page < Math.abs(totalNumber / this.pagination.perPage)
      );
    },
  ...
  shareCheckedList(item: any[]) {
    console.log(item, this.config.columns);
    this.controlNotEmptyChecked = this.config.columns.find(
      (el) => el.checked === true
    )
      ? true
      : false;
  ...
  onDowloadList(list: any) {
    this.onDownloadList.emit(this.config.columns);
  }
  ...
  public selection: any = {
      ...
      if (this.config.selection.selectionType === SelectionType.MULTIPLE) {
        if (this.selection.isSelected(el))
          this.selectedRowsIndexes.splice(
            this.selectedRowsIndexes.indexOf($id),
            1
          );
        else this.selectedRowsIndexes.push($id);
      } else {
    ...
    _getSelectedRows: (): any | Array<any> => {
      ...
      if (
        ...
        this.config.selection.enable &&
        this.config.selection.selectionType === SelectionType.SINGLE
      ) {
        selected = null;
        this.selectedRowsIndexes.forEach(
          (v) => (selected = this.data.find((e) => e.__id === v))
        );
        //if (selected)
        //    delete selected.__id;
      } else {
        selected = [];
        this.selectedRowsIndexes.forEach((v) =>
          selected.push(this.data.find((e) => e.__id === v))
        );
  ...
  sortChanged($event): void {
    this.config.columns.forEach((el) => {
      if (el.columnName === $event.columnName) el.order = $event.order;
      else el.order = 0;
    });
    this.order = $event;
    // Ordina i dati della tabella
    this.data.sort((a, b) => {
      if (a[$event.columnName] === null) return 1; // mette i null alla fine
      if (b[$event.columnName] === null) return -1; // mette i null alla fine
      if (a[$event.columnName] < b[$event.columnName]) {
        return $event.order === 1 ? -1 : 1;
      } else if (a[$event.columnName] > b[$event.columnName]) {
        return $event.order === 1 ? 1 : -1;
      } else {
        return 0;
      }
  ...
  filter($event): void {
    ...
    if (!searchText) {
      ...
      if (lengthChange) this.pagination.change(1);
      return;
    }
    ...
    for (let s in it) {
      ...
      if (typeof el === "object") {
        res = this.check(
          el,
          searchText,
          previousProperty != null ? previousProperty.concat("." + s) : s
        );
        if (res) return res;
      }
      ...
      if (!el.toLowerCase) el = String(el);
  ...
  delete(el: any) {
    ...
    if (!this.selectedRowsIndexes) this.selection._resetSelection();
  }
  ...
  containsNewlineOrTooLong(value: any, lengthLimit: number): boolean {
    return (
      (typeof value === "string" && value.includes("\n")) ||
      (typeof value === "string" && value.length > lengthLimit)
    );
  }
  containsNewline(value: any): boolean {
    return typeof value === "string" && value.includes("\n");
  }
  tooLong(value: any, lengthLimit: number): boolean {
    return typeof value === "string" && value.length > lengthLimit;
  }
  ...
  hasInfo(el: any): boolean {
    if (this.config.info.hasInfo) return this.config.info.hasInfo(el);
  }
  ...
  isEditable(el: any): boolean {
    if (this.config.edit.isEditable) return this.config.edit.isEditable(el);
    else return true;
  }
  ...
  ngOnInit() {
    ...
    // if multiple selection is active, and the list counts only one el
    //the first l is selected by default
    //CR issue 78 conam

    if (
      this.config.selection.selectionType === 1 &&
      this.defaultFirstElSelected === true &&
      this.data &&
      this.data.length === 1
    ) {
      this.selection.onSelection(this.data[0]);
    } else if (
      this.config.selection.selectionType === 1 &&
      this.defaultDataSelected &&
      this.defaultDataSelected.length > 0 &&
      this.config.comparatorField
    ) {
      /// task 97 conam
      //if multiple selection is triggered,
      //and default value selected are passed by the input defaultDataSelected
      this.data.forEach((el, index) => {
        this.defaultDataSelected.forEach((item) => {
          if (
            item[this.config.comparatorField] ===
            el[this.config.comparatorField]
          ) {
            this.selection.onSelection(this.data[index]);
          }
        });
      });
    }
    ...
    //console.log('datatable this.data',this.data);
  }
  ...
  _setSelected() {
    if (
      ...
      let intersection = this.data.filter((a) =>
        this.dataSelected.some(
          (b) =>
            a[this.config.comparatorField] === b[this.config.comparatorField]
        )
      );
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (this.config && this.config.selection && this.config.selection.enable) {
      ...
      // ENG peer per deselezionare dall'esterno
      if (changes["dataSelected"]) {
        ...
        if (current && prev && current.length < prev.length) {
          ...
          let intersection = this.data.filter((a) =>
            differences.some(
              (b) =>
                a[this.config.comparatorField] ===
                b[this.config.comparatorField]
            )
          );
          intersection.forEach((item) => {
    ...
    if (changes["config"]) this._recheckDefaults();
  }
  ...
  _recheckDefaults() {
    ...
    if (null == this.config.filter) this.config.filter = {};
    if (null == this.config.filter.enable) this.config.filter.enable = false;
    if (null == this.config.pagination) this.config.pagination = {};
    if (null == this.config.pagination.enable)
      this.config.pagination.enable = false;
    if (null == this.config.selection) this.config.selection = {};
    if (null == this.config.selection.enable)
      this.config.selection.enable = false;
    if (null == this.config.delete) this.config.delete = {};
    if (null == this.config.delete.enable) this.config.delete.enable = false;
    if (null == this.config.edit) this.config.edit = {};
    if (null == this.config.edit.enable) this.config.edit.enable = false;

    // Ricezione impostazioni opzionali di configurazione paginazione
    if (this.config.pagination && this.config.pagination.enable) {
      if (
        null != this.config.pagination.perPages && // impostato?
        ...
        this.pagination.perPage =
          this.config.pagination.perPages[1] ||
          this.config.pagination.perPages[0];
        this.pagination.endIndex = this.pagination.perPage - 1;
      }
      if (
        undefined !== this.config.pagination.range &&
        typeof this.config.pagination.range === "number"
      ) {
        this.pagination.mRange = this.config.pagination.range;
      }
    }
    if (
      isNaN(+this.config.selection.selectionType) ||
      (+this.config.selection.selectionType !== SelectionType.SINGLE &&
        +this.config.selection.selectionType !== SelectionType.MULTIPLE)
    ) {
      this.config.selection.selectionType = SelectionType.SINGLE;
    }
    if (!this.selectedRowsIndexes) this.selection._resetSelection();
