...
export class VerbaleService {
  ...
  getVerbaleById(idVerbale: number): Observable<VerbaleVO> {
    ...
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/dettaglioVerbale";
  ...
  salvaVerbale(verbale: VerbaleVO): Observable<number> {
    let body = verbale;
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/salvaVerbale";
    return this.http.post<number>(url, body);
  }

  ricercaSoggetto(minSoggetto: MinSoggettoVO) {
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/ricercaSoggetto";
  ...
  eliminaVerbale(idVerbale: number) {
    let params = new HttpParams().set("idVerbale", idVerbale.toString());
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/eliminaVerbale";
  ...
  eliminaSoggettoByIdVerbaleSoggetto(idVerbaleSoggetto: number) {
    let params = new HttpParams().set(
      "idVerbaleSoggetto",
      idVerbaleSoggetto.toString()
    );
    var url: string =
      this.config.getBEServer() +
      "/restfacade/verbale/eliminaSoggettoByIdVerbaleSoggetto";
    return this.http.get(url, { params: params });
  }
  ...
  getAllegatiByIdVerbale(idVerbale: number): Observable<RiepilogoAllegatoVO> {
    let params = new HttpParams().set("idVerbale", idVerbale.toString());
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/getAllegatiByIdVerbale";
  ...
  salvaAzioneVerbale(salvaAzioneRequest: SalvaAzioneRequest) {
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/salvaAzioneVerbale";
    return this.http.post(url, salvaAzioneRequest);
  }

  listaStati(): Observable<Array<any>> {
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/getStatiManuali";
  ...
  getUtenteRuolo(idVerbale: number): Observable<UtenteVO> {
    let params = new HttpParams().set("idVerbale", idVerbale.toString());
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/getUtenteRuolo";
    return this.http.get<UtenteVO>(url, { params: params });
  }

  salvaStatoVerbale(salavaStatoFascicolo: SalvaStatoRequest) {
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/salvaStatoManuale";
    return this.http.post(url, salavaStatoFascicolo);
  }

  getVerbaleSoggettoByIdSoggetto(
    idSogg: number,
    recidivo?: boolean
  ): Observable<RecidivoVo> {
    let params = new HttpParams().set("idSoggetto", idSogg.toString());
    var url: string =
      this.config.getBEServer() +
      "/restfacade/verbale/getVerbaleSoggettoRaggruppatoPerSoggettoByIdSoggetto";
    if (recidivo) {
      params = new HttpParams()
        .set("idSoggetto", idSogg.toString())
        .set("recidivo", recidivo.toString());
    }
  ...
  salvaScrittoDifensivo(
    ...
    var url: string =
      this.config.getBEServer() +
      "/restfacade/scrittoDifensivo/salvaScrittoDifensivo";
    let form = new FormData();
    form.append(
      "data",
      JSON.stringify(salvaScrittoDifensivoRequest, (k, v) => {
  ...
  setRecidivoVerbaleSoggetto(setRecidivoVerbaleSoggetto: any) {
    var url: string =
      this.config.getBEServer() +
      "/restfacade/verbale/setRecidivoVerbaleSoggetto";
    return this.http.post(url, setRecidivoVerbaleSoggetto);
  }

  getScrittoDifensivoById(
    idScrittoDifensivo: number
  ): Observable<ScrittoDifensivoVO> {
    var url: string =
      this.config.getBEServer() +
      "/restfacade/scrittoDifensivo/dettaglioScrittoDifensivo";
    let params = new HttpParams().set(
      "idScrittoDifensivo",
      idScrittoDifensivo.toString()
    );
  ...
  uploadImportiSogg(idVerbale: number, importoVerbale: number) {
    ...
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/modificaImportoSoggetti";
    let params = new HttpParams()
      .set("idVerbale", idVerbale.toString())
      .set("importoVerbale", importoVerbale.toString());
    return this.http.post(url, null, { params: params });
  }

  ricercaScrittoDifensivo(
    ricercaScrittoDifensivoRequest: RicercaScrittoDifensivoRequest
  ): Observable<Array<ScrittoDifensivoVO>> {
    var url: string =
      this.config.getBEServer() +
      "/restfacade/scrittoDifensivo/ricercaScrittoDifensivo";
    return this.http.post<Array<ScrittoDifensivoVO>>(
      url,
      ricercaScrittoDifensivoRequest
    );
  }

  getFieldValuesProvaDelPagameto(idAllegato: number) {
    var url: string =
      this.config.getBEServer() + "/restfacade/allegato/getFieldsByIdAllegato";
    let params = new HttpParams()
  ...
  updateProvaDelPagameto(idAllegato: number, body: SalvaAllegatoRequest) {
    var url: string =
      this.config.getBEServer() + "/restfacade/verbale/modificaAllegatoVerbale/" + idAllegato.toString();
